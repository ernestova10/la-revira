...
<head>
    ...
    <title>Hermandades - La Revirá</title>
...
<body class="bg-custom-white font-sans text-gray-900">
    ...
    <main class="container mx-auto px-6 py-12 max-w-6xl">
        ...
        <div class="text-center max-w-3xl mx-auto mb-16">
            <h1 class="text-3xl font-bold text-gray-800 mb-6">Hermandades de la Semana Santa de Sevilla</h1>
            <p class="text-gray-600 leading-relaxed">
                Las hermandades y cofradías son el pilar fundamental de la Semana Santa sevillana. Cada una posee una historia propia, un patrimonio artístico único y una profunda vinculación con su barrio y sus devotos. Durante la Semana Santa, las hermandades realizan estación de penitencia, organizando procesiones en las que participan nazarenos, costaleros y músicos, manteniendo vivas tradiciones que se transmiten de generación en generación.
                En esta sección podrás conocer algunas de las hermandades más representativas de Sevilla, su historia, sus pasos procesionales y la forma de participar en ellas.
            </p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($hermandades as $hermandad)
            <div class="bg-custom-blue rounded-xl overflow-hidden shadow-lg hover:scale-105 transition-transform duration-300">
                <img src="{{ asset($hermandad->imagen_tarjeta) }}" 
                    alt="{{ $hermandad->nombre }}" 
                    class="w-full h-56 object-cover">
                
                <div class="p-6">
                    <h3 class="text-white text-xl font-bold">{{ $hermandad->nombre }}</h3>
                    <p class="text-gray-400 text-sm mb-4">{{ $hermandad->sede }}</p>
                    
                    <a href="{{ route('hermandad.show', $hermandad->slug) }}" class="inline-block bg-white text-custom-blue px-6 py-2 rounded-sm font-bold text-sm uppercase tracking-wider hover:bg-gray-200 transition-colors">
                        Ver hermandad
                    </a>
                </div>
            </div>
